...
export default function VoxPopuli({ comments }: { comments: any[] }) {
  return (
    <aside className="w-80 bg-[#1A2A2D]/90 backdrop-blur-xl rounded-[2.5rem] border border-white/5 p-6 flex flex-col h-full shadow-2xl">
        ...
        <AnimatePresence initial={false}>
          {[...comments].reverse().map((c) => (
            <motion.div
              ...
              className={`p-4 rounded-2xl border bg-black/20 ${
                c.sentiment === 'good' ? 'border-green-500/30' : 
