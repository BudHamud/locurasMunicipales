...
export default function LeaderCard({
  ...
  return (
    <motion.div
      onClick={onSelect}
      whileHover={{ y: -5 }}
      className={`relative bg-white border-[6px] rounded-[3.5rem] p-8 flex flex-col gap-6 min-h-[450px] transition-all cursor-pointer
        ${isSelected 
    ...
    >
      ...
      <div className="flex flex-row gap-8 flex-1">
        ...
        {/* COLUMNA IZQUIERDA */}
        <div className="flex flex-col w-56 shrink-0 items-center justify-between">
          <div className="relative w-32 h-32 flex-shrink-0">
            ...
            <div className={`absolute inset-2 bg-slate-50 border-4 rounded-full flex items-center justify-center text-5xl shadow-inner z-10 transition-colors
          ...
          <div className={`relative border-2 rounded-2xl p-4 w-full transition-colors mt-4
            ...
            <div className={`absolute -top-3 left-4 text-[8px] font-black px-2 py-0.5 rounded-full
              ${isSelected ? "bg-arg-gold text-white" : "bg-arg-darkblue text-white"}`}>
              CV REVISADO
            </div>
            <p className="text-[11px] font-bold text-arg-darkblue/70 leading-tight italic text-center">
              {career}
            </p>
          </div>
        </div>
